...
「プラクティス」はサーバーなしで動く。出撃準備から、全8面の「ターゲットチャレンジ」かマップ・武器を選ぶ「自由練習」に進む。チャレンジは規定弾数で的をすべて壊すと次の面が解放され、最少弾数をブラウザに保存する。ルールと画面遷移は[設計書37](docs/design/37-practice.md)を参照。
