...
export function validateAvatar(){
 ...
 if(manifest.avatarPortraitVersion!==2||manifest.model!=='gpt-image-2.5-sunburst')throw new Error('Portrait format/model mismatch');
 const images=new Map();
 for(const layer of manifest.layers){
  const image=inspectPng(readFileSync(root+'/'+layer.file),{pixels:true});
  if(image.width!==manifest.frameSize[0]*manifest.frameCount||image.height!==manifest.frameSize[1])throw new Error('Atlas dimensions');
  images.set(layer.id,image);
  if(readFileSync(root+'/'+layer.source).subarray(0,2).toString()!=='PK')throw new Error('ORA missing');
  for(const metric of manifest.metrics[layer.id]){
   const hash=sourceDigest(readFileSync(root+'/generated/'+metric.sourceSheet+'.png'));
   if(metric.sourceHash!==hash)throw new Error('Source provenance mismatch');
  }
 }
