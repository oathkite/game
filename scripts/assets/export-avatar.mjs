...
const root=resolve('assets/workbench/avatar-kita-v1'),size=[64,64],scale=.07;
...
function ora(layer,png){
 ...
 writeFileSync(join(dir,'stack.xml'),`<image version="0.0.3" w="192" h="64"><stack><layer name="${layer}" src="data/layer.png" x="0" y="0" opacity="1.0" visibility="visible" composite-op="svg:src-over"/></stack></image>`);
...
for(const layer of ['body','glasses','scarf']){
 ...
 const sheet=registration.sheets?.[layer]??(layer==='body'?'poses':layer),bytes=readFileSync(join(root,'generated',sheet+'.png')),frames=[];metrics[layer]=[];
 for(let i=0;i<3;i++){
  ...
  const origin=pose.origins?.[layer]??[Math.round(32+(left+x-pose.sourceFoot[0])*layerScale)+offset[0],Math.round(60+(top+y-pose.sourceFoot[1])*layerScale)+offset[1]];
  ...
  frames.push({input,left:i*size[0]+origin[0],top:origin[1]});metrics[layer].push({region:r,sourceBounds:[left+x,top+y,w,h],outputSize:outSize,outputOrigin:origin,scale:layerScale,sourceSheet:sheet,sourceHash:sourceDigest(bytes)});
 }
 const raw=await sharp({create:{width:192,height:64,channels:4,background:'#00000000'}}).composite(frames).raw().toBuffer();
 const png=indexed(raw,192,64);writeFileSync(join(root,layer+'.png'),png);ora(layer,png);
}
const manifest={avatarPortraitVersion:1,id:'kita-personal-portraits',status:'draft',availability:{scope:'owner-only',grantKey:'kita-personal'},model:'gpt-image-2.5-sunburst',frameSize:size,foot:[32,60],scale,poses:registration.poses.map((p,i)=>({id:p.id,label:p.label,frame:i,faceArea:p.faceArea,eyeAreas:p.eyeAreas,headAnchor:p.headAnchor,neckAnchor:p.neckAnchor})),layers:['body','scarf','glasses'].map(id=>({id,file:id+'.png',source:id+'.ora'})),metrics,animationStatus:'representative-still-poses-only',humanReview:'pending'};
