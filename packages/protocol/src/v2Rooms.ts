...
export const roomInputSchema = z.union([
  ...
  z.object({ type: z.literal("room.spectate"), invite: z.string().uuid().optional(), build: clientBuildSchema.optional(), roomId }).strict(),
  z.object({ type: z.literal("room.create"), build: clientBuildSchema.optional(), profile: lobbyProfileSchema }).strict(),
  z.object({ type: z.literal("room.join"), invite: z.string().uuid().optional(), build: clientBuildSchema.optional(), roomId, profile: lobbyProfileSchema }).strict(),
