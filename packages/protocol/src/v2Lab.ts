...
export const labFrameSchema = z.object({
  ...
  map: z.object({ id: z.string().min(1).max(128), version: z.number().int().positive(),
    ...
    solidColumns: z.array(z.array(z.tuple([z.number().int().min(0).max(225), z.number().int().min(0).max(225)])).max(225)).max(500).optional(),
  }).refine(map => map.surface.length === map.width && map.surface.every(y => y <= map.height) && (!map.solidColumns || (map.solidColumns.length === map.width && map.solidColumns.every(runs => runs.every(([start, end], i) => start < end && end <= map.height && (i === 0 || start >= runs[i - 1]![1])))))),
