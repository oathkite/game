...
/** 1 発の武器の着弾セル。外れなら null */
const impactOf = (r: ShotResult) => r.impacts[0]?.cell ?? null;
/** 1 発の射撃が各席に与えたダメージの合計 */
const damageOf = (r: ShotResult): [number, number] => [damageDealtTo(r, 0), damageDealtTo(r, 1)];
/** 1 発の武器の弾道 */
const pathOf = (out: ShotOutcome) => out.paths[0]!.points;
...
describe("1 発の処理", () => {
  ...
  it("平地で前方に撃てば地面に当たり、地形が削れる", () => {
    ...
    expect(out.result.impacts[0]!.terrainOp).toEqual({ cx: impact.x, cy: impact.y, radius: BLAST_RADIUS });
...
describe("鏡像の対称性", () => {
  const mirrorResult = (r: ShotResult, width: number): ShotResult => ({
    ...
    impacts: r.impacts.map((i) => ({ ...i, cell: { x: width - 1 - i.cell.x, y: i.cell.y }, terrainOp: { ...i.terrainOp, cx: width - 1 - i.terrainOp.cx } })),
