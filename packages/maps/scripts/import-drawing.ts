...
// JSON の形: { "name": "識別子", "label": "表示名", "spawns": [x0, x1], "spawnFromY": [y0, y1], "columns": "top-bottom,...;..." }
// spawnFromY は省略できる（0 = 上から最初の地面）。天井の下に置くときに、天井より下の y を指定する
...
const savedSchema = z.object({
  ...
  spawnFromY: z.tuple([z.number().int().min(0), z.number().int().min(0)]).optional(),
  columns: z.string(),
});
...
const drawing: Drawing = { name: saved.name, spawns: saved.spawns, columns: decodeColumns(saved.columns), ...(saved.spawnFromY ? { spawnFromY: saved.spawnFromY } : {}) };
...
const ${saved.name}: MapDefinition = {
  name: "${saved.name}",
  spawns: [${saved.spawns[0]}, ${saved.spawns[1]}],${saved.spawnFromY ? `\n  spawnFromY: [${saved.spawnFromY[0]}, ${saved.spawnFromY[1]}],` : ""}
  build: () =>
    slabs([
${body}
    ]),
