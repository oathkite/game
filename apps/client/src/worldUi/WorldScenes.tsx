...
type Scene = "start" | "lobby" | "settings" | "battle" | "result";
export const WorldScenes = () => {
  ...
  return <div className={`world-ui world-scene-${scene} ${closing ? "world-closing" : ""}`} style={{ backgroundImage: `url(${background})` }}>
    {scene === "battle" ? <CameraPrototype worldArt onExit={exit} onResult={finish} /> : <>
...
const Lobby = ({ go }: { readonly go: (scene: Scene) => void }) => {
  ...
  return <section className="world-lobby">
    ...
    <PixelPanel className="world-loadout">
      ...
      <p>ふたりで交代して遊ぶプラクティス。<br />同じ端末で移動と射撃を試せます。</p>
    </PixelPanel>
    <footer><PixelButton onClick={() => go("start")}>タイトルへ</PixelButton><PixelButton onClick={() => go("battle")}>プラクティスへ</PixelButton></footer>
  </section>;
};
