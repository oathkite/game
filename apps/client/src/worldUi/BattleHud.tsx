...
export const BattleRoster = ({ players, actorId, upcomingPlayerIds = [], clock, wind, onMenu }: { readonly players: readonly HudPlayer[]; readonly actorId: string; readonly upcomingPlayerIds?: readonly string[]; readonly clock: ReactNode; readonly wind: number; readonly onMenu: () => void }) => { const { t } = useLanguage();
  const seats = (group: readonly HudPlayer[]) => <div className="battle-seats">{group.map(p => <div className={`battle-seat ${p.id === actorId ? "is-actor" : ""}`} key={p.id} style={{ "--team": teamColor(p.team) } as CSSProperties} aria-label={`${p.name}, ${t("{color}チーム", { color: t(teamColorName(p.team)) })}, HP ${p.hp}${p.id === actorId ? ` ${t("手番")}` : ""}`}>
    ...
    <div className="battle-seat-portrait" aria-hidden="true"><TankPortrait colors={p.colors} /></div><span className="battle-team-dot" /><strong>{p.name}</strong><div className="battle-hp"><i style={{ width: `${Math.max(0, Math.min(100, p.hp))}%` }} /></div>
  </div>)}</div>;
...
export const BattleConsole = (p: Props) => { const { t } = useLanguage(); return <footer className={`battle-console ${p.children ? "has-touch" : ""}`}>
  <div className="battle-self" style={{ "--team": teamColor(p.player?.team ?? 0) } as CSSProperties}><div className="battle-self-portrait" aria-hidden="true"><TankPortrait colors={p.player?.colors} /></div><div className="battle-self-info"><strong>{p.player?.name ?? "—"}</strong><div className="battle-hp"><i style={{ width: `${Math.max(0, Math.min(100, p.player?.hp ?? 0))}%` }} /></div><span>{p.player?.hp ?? 0}/100</span><MovementReserve steps={p.steps} /></div></div>
