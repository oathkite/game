...
export const BattleConsole = (p: Props) => { const { t } = useLanguage(); return <footer className={`battle-console ${p.children ? "has-touch" : ""}`}>
  <div className="battle-self" style={{ "--team": teamColor(p.player?.team ?? 0) } as CSSProperties}><div className="battle-self-portrait" aria-hidden="true"><TankPortrait /></div><div className="battle-self-info"><strong>{p.player?.name ?? "—"}</strong><div className="battle-hp"><i style={{ width: `${Math.max(0, Math.min(100, p.player?.hp ?? 0))}%` }} /></div><span>{p.player?.hp ?? 0}/100</span><MovementReserve steps={p.steps} /></div></div>
