...
export const BattleMenu = ({ close, surrender, exit, finished, report, diagnostics, latency, spectator = false, activeTurn = false }: { readonly activeTurn?: boolean; readonly latency?: number | null; readonly report?: ReportOptions; readonly diagnostics?: string; readonly spectator?: boolean; readonly close: () => void; readonly surrender: () => void; readonly exit: (() => void) | undefined; readonly finished: boolean }) => {
  const { t } = useLanguage();
  ...
  return <dialog ref={dialog} className="battle-menu-panel" aria-label={t("対戦設定")} onCancel={e => { e.preventDefault(); close(); }}>
    {activeTurn && !finished && !spectator && <strong className="battle-turn-notice" role="status">{t("あなたの手番です")}</strong>}
