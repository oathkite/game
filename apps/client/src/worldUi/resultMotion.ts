...
/** 勝者の機体が跳ねる長さ（C6）。行が出きった後（行の間隔 + 90 ms）に始まるので、行が多いと RESULT_MOTION_MS を越える。 */
export const RESULT_WINNER_HOP_MS = 600;
...
export const resultMotionEndMs = (rows: number): number =>
  Math.max(RESULT_MOTION_MS, Math.max(0, rows - 1) * resultRowStepMs(rows) + RESULT_ROW_MS + RESULT_WINNER_HOP_MS);
