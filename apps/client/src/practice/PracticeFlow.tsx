...
type Page = { readonly kind: "menu" } | { readonly kind: "free" } | { readonly kind: "challenge"; readonly index: number; readonly attempt: number };
...
export const PracticeFlow = ({ profile, onProfileChange, onExit, onFreeStart }: Props) => {
  ...
  useBrowserBackAction(page.kind === "free", back);
  ...
  if (page.kind === "challenge") {
    ...
    return <ChallengeScreen key={`${stage.id}-${page.attempt}`} stage={stage} profile={profile} onProfileChange={onProfileChange} best={progress[stage.id]} onBack={back}
  ...
  return <PracticeMenu progress={progress} saved={saved} onStage={(index) => setPage({ kind: "challenge", index, attempt: 0 })} onFree={() => setPage({ kind: "free" })} onBack={onExit} />;
};
