...
type Props = { readonly cpu?: boolean; readonly profile: Profile; readonly onProfileChange: (p: Profile) => void; readonly onStart: (map: MapName | "random") => void; readonly onBack: () => void };
export const FreePracticeSetup = ({ cpu = false, profile, onProfileChange, onStart, onBack }: Props) => {
  ...
  return <main className="menu-shell practice-menu"><div className="menu-content practice-content">
    ...
    </div><footer className="menu-actions"><button className="primary-action" onClick={() => onStart(map)}>{t(cpu ? "CPU戦をはじめる" : "自由練習をはじめる")}</button><button onClick={onBack}>{t("プラクティスへ戻る")}</button></footer></main>;
};
