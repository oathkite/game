...
export const createTerrainLayer = (mask: TerrainMask, art?: CanvasImageSource): TerrainLayer => {
  // Physics stays at one cell per pixel. Artwork gets eight texels per cell so
  ...
  if (art && tileContext) { tileContext.imageSmoothingEnabled = false; tileContext.drawImage(art, 0, 0, tile.width, tile.height); }
  ...
  const paint = (m: TerrainMask): void => {
    ...
    if (pattern) {
      ...
      for (let y = 0; y < m.height; y++) for (let x = 0; x < m.width; x++) {
        if (m.cells[y * m.width + x] && (y === 0 || !m.cells[(y - 1) * m.width + x])) ctx.fillRect(x * scale, y * scale, scale, 2);
      }
    }
  };
  paint(mask);
