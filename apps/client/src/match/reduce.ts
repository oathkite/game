...
export const reduce = (view: MatchView, message: ServerMessage, options: ReduceOptions, replayId: number): Reduced => {
  switch (message.type) {
    case "match.setup": {
      ...
      return just(
        { ...EMPTY_VIEW, mapId: message.mapName, phase: "loading", mask, players, mySeat: options.mySeat, spectator: options.spectator, currentSeat: message.firstSeat, turnLimit: message.turnLimit },
        "match.ready",
      );
    }
    case "conn.state":
