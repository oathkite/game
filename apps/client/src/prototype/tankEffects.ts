...
export const createTankEffects = () => {
  let hp: number | null = null, falling = false, destroyedAt = -Infinity, landedAt = -Infinity, hitAt = -Infinity;
  return (pose: Pose, now: number, reducedMotion = false): readonly Effect[] => {
    if (hp !== null && pose.hp < hp) hitAt = now;
    if (pose.hp <= 0 || (hp !== null && pose.hp > hp)) hitAt = -Infinity;
    ...
    if (pose.hp > 0 && now - hitAt < 300) add("spark", frameAt(now - hitAt, [40, 100, 180, 300]), 115, 107, 1);
