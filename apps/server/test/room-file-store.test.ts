...
it("reopens a persisted room with reconnect grace and preserves its private session", async () => {
  ...
  try {
    ...
    const state = reduceRoom(createRoomState("ABCDEF"), "c1", { type: "room.create", profile: { nickname: "Kero", loadout: ["cannon", "digger"] } }, 1000, id).state;
    ...
    const resumed = reduceRoom(restored!, "c2", { type: "room.resume", token: id.token }, 2100, id);
