...
export const battleFrame = (room: RoomState, now: number): LabFrame => {
  ...
  return { type: "lab.frame", ...(state.phase === "finished" && state.stats ? { stats: state.stats } : {}), build: state.build, serverTime: now, eventSeq: state.movement.eventSeq, matchId: state.matchId,
