...
- production Chromium entry4件通過。岩盤/苔変更後タイトル1,861,274B、練習累計5,416,375B。2MB/8MB条件内。8人オンラインの転送量は地形差替前の値なので別に再測定を残す。
- design30の「風0」「worldは配信しない」を現在の通常入口・サーバー風へ更新。design33の未実装一覧から実装済みの日英/イントロ/音/診断を区別し、公開gateは維持。
...
### HUDポートレート接続と再開時検証（2026-09-11）

- 生成した汎用カエルの顔を参加者一覧と自機パネルに表示。戦場のsprite sheetや機体選択のプレビューは変更していない。原案への最終一致・人間の素材承認は未完。
- 原PNGを保存し、可逆WebPとmanifestへ登録。12枚のRGBA/RMS・hash検証、client/e2e typecheck通過。
...
- 設計21の次の参加者表示を接続。サーバーのturnRing/cursorから脱落者と現在手番を除いて最大3名を配信し、HUD参加者欄へ小さな1〜3の予告を表示。1対1では次の相手1名のみ、決着後は予告なし。
- 旧frameを読む互換性のため通信fieldはoptional。新サーバーは常に計算して送信。色はチーム、数字は手番予告に限定。日本語・英語の読み上げラベルを追加。
- 折り返し・脱落・1対1・決着の単体2件、翻訳網羅、全workspace型検査、server68件が通過（別edge用2件skip）。
- 配信build/local edgeのChromium独立8人テストで予告1位→実際の次手番の一致、射撃共有、降参決着、部屋復帰が通過（41.5秒）。今回の転送量各6,326,290B。

### 設定中の手番通知（2026-09-11）

- 設計21.6の「設定paneは閉じず通知する」を補完。オンラインの設定dialogで自分の操作手番に限りYour turn/あなたの手番ですをrole=statusで表示。観戦・終了時には表示しない。dialogや入力フォーカスを自動で閉じたり動かしたりしない。
- 独立8人E2Eへ、次手番の参加者が設定を開く→射撃共有→手番移行→dialog維持・通知→ボタンで復帰を追加。変更前は通知なしで失敗し、変更後は39.6秒で通過。
- client/e2e型検査、翻訳網羅検査通過。

### 個人成績の集計とリザルト表示（2026-09-11）

- 発射数と敵/味方/自分への実HPダメージをサーバーで累積。各射撃の複数着弾を合算して射撃前HPで上限を設け、オーバーキルを実ダメージへ加えない。降参やリングアウトだけでは与ダメージを捏造しない。
- 成績はBattleSessionと既存snapshotに保持し、finished時だけframeへ送信。既存snapshotにstatsがない場合は成績を非表示とし、途中から不完全な成績を作らない。新しい試合は全員0で開始。
...
### 原案の風小窓への調整（2026-09-11）

- desktop-refined-v4原案を再確認し、風表示を濃紺の面に雲・方向矢印・数値を並べるSVGへ変更。左右の向きと絶対値を分離し、無風は横棒。物理値や単位は変更せず、m/sなどの誤った単位を追加しない。
- shared BattleRosterで練習/オンライン双方へ適用。日英の読み上げラベルを追加。PCと横向き844実画面で背景上の視認性とHUDとの非重複を確認。
- client型検査・翻訳検査、world scenes/地形6E2Eが通過（20秒）。UI全体の原案一致は未完。

### パワーメーターの原案への調整

- 未充填部分も青灰色の100分割として表示し、充填済み部分は従来の連続した値に合わせて緑から黄・橙へ塗り分ける。1段階の目盛りと10段階ごとの長い目盛り、現在値の数値を維持し、位置を示す三角形の指針を追加。
- client typecheck、diff check、world scenes / battle input のブラウザテスト8件が成功（28.0秒）。1440pxと667pxの実画像、実際のSpace充填中の画像で区切りと指針を確認。スクリーンショットはローカル test-results に保存。
- UI全体の原案への一致、ゲーム内パイロットの素材整合、実機・公開環境での検証は引き続き未完了。

### 武器選択を既存の弾素材へ統一

- BattleConsole の8種類の武器アイコンを、実際の飛翔体と同じ runtime/tanks-v1/projectile-*.png へ変更。SVG viewport で透明余白を除き、元画像の変更・再生成は行っていない。
- トリプル弾は3発、マルチ弾はシミュレーション仕様（3方向×3斉射）に合わせ9発を配置。装備名のaria-label、Q/E、選択状態の枠は既存のまま。
- client typecheck / diff check成功。Chromiumで全8種の装備の手番まで待ち、各画像URLとQ/E切り替えを確認。1440pxで4組の実画像、667px touchでマルチ弾9発・ドリル選択・発射ボタンを確認。画像はローカル test-results/weapon-icons-*.png。
- これは武器アイコンの局所的な調整。UI全体の原案一致や未完了の公開前検証を完了扱いにはしない。

### 全参加者のリザルト表情

- 設計21の全員のキャラクター反応をオンラインのリザルトへ追加。既存の緑のパイロットを参照し、内蔵画像生成で喜び・落胆の2表情を作成。元PNGを保存しlossless WebPで登録した（world runtimeは13素材）。
- 確定したサーバーの勝利teamIdで表情を選択し、HPや生存状態を勝敗の代用にしない。引き分けは通常の表情、ongoingは非表示。チーム色の下線、名前、勝敗の読み上げを用意。勝利時の短い動きはreduced-motionで停止する。
- client typecheck、翻訳coverage、assets:check、diff check成功。production Chromiumの8クライアント4v4 E2Eが成功（57.1秒）。全8画面で勝者4人・敗者4人を確認し、成績共有と全員のロビー帰還まで検証。実画像で表情とチーム色の対応を確認。
- この追加についてFirefox/WebKit・実機・引き分けのブラウザ検証はまだ実施していない。プラクティスのリザルトは従来のTankPortraitで、全体のUI整合も継続作業。

### プラクティスのリザルト表現を共通化

- プラクティス終了時に翻訳済み文字列だけを渡す方式を、参加者・確定勝敗のデータへ変更。オンラインと同じResultPlayersを使い、2人の名前・チーム色・喜び／落胆の表情を表示する。タイトルは描画時の言語で生成。
- 共通の表情スタイルをresultPlayers.cssへ分離し、プラクティスではカードを拡大。結果用モジュールはlazyで読み込む。オンラインの帰還カウントダウンには紺色の背景を付け、背景画像から区別できるようにした。
- client typecheck/build、resultPlayersの静的レンダリング2件（撃破された勝利チームの味方、引き分け／進行中）、翻訳coverage、diff check成功。world scenes/battle inputの8件成功（29.3秒）。1440pxの結果画像で2人の表情・名前・再プレイボタンを確認し、再プレイまで検証した。
- この変更後のオンライン3ブラウザ再検証とモバイルの結果画面確認は未実施。全体の原案整合・実機等の公開前検証も継続。

### リザルトのモバイル表示と3ブラウザ検証

- 667pxの結果画面で参加者が7人＋1人に折り返されることを追加E2Eで再現し、700px以下は4列×2段へ修正。
- 終了後は結果画面を全画面にし、背後の対戦用roster・console・fieldを非表示、縦向き回転案内も非表示にした。勝敗カード・成績・帰還操作へ表示領域を使う。
- production build / local edgeでChromium・Firefox・WebKitそれぞれ独立8contextの4v4を検証。全員の表情・成績共有、667×375で4列/2段と帰還ボタン到達、390×844で回転案内に遮られず帰還できること、全員の部屋復帰が成功（3件、3.2分）。画像も確認済み。
- 初回WebKitは追加の表示検証を含め120秒のテスト枠を超過したため、全体枠を180秒へ変更。再実行は約2.1分で成功。ゲームの時間制限や通信SLOを変更したものではなく、実機性能の合格を示すものでもない。
- 最終実行の初回対戦までの最大encoded transferは6,330,613B。型チェック・diff checkも成功。実機/地域/DOと正式素材承認、UI全体の原案整合は引き続き未完了。

### 岩盤と草地の原案への調整

- 内蔵画像生成で原案の崖を参照したterrain-cliff-v2を作成し、従来の細かな丸い岩から大きな角張った岩盤へ変更。元画像を保存しlosslessでruntimeへ登録。地形WebPは1,520,170Bから1,267,828Bへ減少。
...
## オンラインの中央タイマー検証（4aa427e）

- eight-players.spec.tsへ残り秒数とリング比率、8画面すべての射撃再生中の空リング、次手番でのリング復帰を追加。
- Chromium/Firefoxは4v4の入室・射撃・成績・全員帰還まで成功。WebKit初回は初期読込/転送量計測後に手番が進んで古い次手番を参照し、操作可能人数の検査で失敗。手番開始直後（18秒以上）を待つ試験手順へ修正し、WebKit再試験は成功（2.8分）。ゲームの20秒期限や時計は変更していない。
- オンライン累計encoded transfer最大：Chromium 6,345,906B、Firefox 6,315,635B、WebKit 6,316,126B。すべて8MB以内。配信build/local SQLite DOによる結果で、実地域/実機性能とは別。
- 最初の別ポート5188試験は許可オリジンとの不一致で入室前に失敗。既定5186へ戻して上記を実行し、終了後は開発サーバー（backend 8796）を復旧。

## オンラインのカメラボタン統一

- network-focusもプラクティスと同じ濃紺・淡色アイコン・focus outlineを共用。844×390のオンライン画面を目視確認。
- rooms.spec.ts（2人、PC/タッチ、入室→装備選択→対戦→準備復帰）成功、13秒。初回は開発用8796サーバーが停止しており入室前に失敗。待受なし/HTTP 000を確認し、local Wranglerを8796で再起動後に成功。
- 開発接続：client 5186 → backend 8796。新しいローカル保存先はapps/server/.keropod/dev-world。既存保存領域は削除していない。

## モバイル操作ボタンのコックピット統一

- 移動/角度ボタンを濃紺と淡色アイコン、発射を琥珀色の操作面へ統一。44px以上の操作高さと既存pointer操作を維持。タッチUIでは装備番号1/2、物理キー入力後はQ/Eを表示する。
- PCキーボード/タッチ切替・4viewportの計6件成功（27.4秒）、オンライン2人の装備/射撃/再接続/準備復帰も成功（12秒）。667pxプラクティスと844pxオンラインの画像を目視確認。client/e2e型検査とdiff check成功。

## 狭いフィールドの名前/HPプレート

- 表示中の機体の名前/HPプレートをフィールド内側4pxへ制限し、上部帯による欠けを防止。画面外の機体は端に固定せず、自然なラベル座標を別に保持してカメラ往復時の累積補正を防ぐ。
- 境界/画面外/通常位置の3単体テスト成功。実rendererのカメラ往復・復帰と4viewportシーンの5件成功（19.1秒）。667pxの画像で名前とHPの全体表示を目視確認。client/e2e型検査とdiff check成功。
- 複数ラベル同士の重なりや風/ミニマップとの重なりを全配置で解消する機能ではない。今回の変更はフィールド端による欠けを対象とする。

## 代表4編成のブラウザ完走

- 22.8の1v7、8人FFA、2v2v2、1v1v1v2をformations.spec.tsへ追加。各参加者は独立contextでUIから入室/チーム設定/ready。全員の受信frameで本人のTeamIdを確認し、実キーボード射撃の再生を共有する。
- 最終チーム以外がUIから降参し、全員の勝敗/成績一致、勝利・敗北の人数、同じ部屋への帰還を確認。射撃だけの決着や全編成の体感バランスの証明ではない。
- 開発版Chromium4件成功（2.2分）。配信build+local SQLite DOのChromium4件も成功（1.5分）。e2e型検査とdiff check成功。production-rooms.config.tsに追加し、他の2engineでも実行できる構成。今回の実測はChromiumのみ。
- 試験後は開発client 5186→backend 8796へ復旧。
