...
最新の配信統合試験: 2026-09-12、画像地形3マップ・葦の丘v4・計器盤・シーン往復を含め、開幕中の練習降参を修正後に111件が成功（3browser各37件）。初期タイトル最大1,863,651B、初回練習まで最大7,443,417B。詳細はopening-exit-2026-09-12.md。過去のCI・実機・実DO検証とは区別する。

標準開発起動も新Room APIへ修正し、`pnpm dev`からの2人対戦・地形破壊・再接続が成功。詳細は[ローカル起動検証](local-startup-2026-09-12.md)。

オンラインのresize時に長押しが残る不具合を修正。独立2contextの画面回転・入力復帰を3browserで確認し、client243件も成功。詳細は[入力解除検証](resize-input-2026-09-12.md)。

原案に合わせて計器盤の金属縁と移動残量計を追加し、角度計の下端切れを修正。配信6件・計器unit7件・型検査が成功。[計器盤の実画面](evidence/ui/cockpit-reserve-2026-09-12/README.md)を保存。

計器盤変更後のオンライン検証: 3browserで8人の射撃共有・結果一致・全員帰還が成功。最大転送7,653,325B。モバイルの移動残量とサーバー確定値の一致も3browserで成功。[詳細](cockpit-online-2026-09-12.md)。
...
| 画像地形の通常3マップ展開 | rock-arch v2、reed-hills v4、moss-valley v3を登録。画像alphaと共通初期solid列・保存復元・版別素材を接続 | 葦の丘は統合射程試験で開始位置を修正しv4へ更新。画像はv3と同じ。旧版の保存試合は旧地形・配置を維持。詳細はintegrated-review-2026-09-12.md |
| 苔の谷v3の8人対戦 | Chromium/Firefoxの成功ログに加え、WebKitの8context試験を再実行して完走。最大7,103,646B | 帰還要求の並行化で待ち時間を短縮。試合内容と全員待機/帰還の確認を維持。evidence/ui/moss-valley-eight-2026-09-12 |
| 再接続時のカメラ初期位置 | 7377233で初回描画から手番へfocus。修正前の座標ずれを再現し、修正後3browser成功 | 通常手番のイージングと開始演出を維持。camera-reconnect-2026-09-12.md |
...
最終UIの原案はdesign/references/battle-hud-approved.pngに保存。後続指定との比較基準はdesign/35-final-ui-reference.mdを参照。円形角度計の金属縁・文字盤陰影を調整し計器4テスト・client型チェック・ローカル実画面を確認したが、全体の最終一致を証明するものではない。
...
## 2026-09-12 地形・カメラ調整後の再検証

- 円形破壊の黒いstrokeを撤去。透過モヤを除去したrock-arch-v2から当たり判定を再生成。
- 標準表示倍率を12→9（75%）へ変更。明示保存された等倍設定は維持。UI寸法は変更しない。
- client 241件、maps 86件、alpha加工1件、canvas境界/復元1件とclient型検査成功。
- 配信buildでChromium/Firefox/WebKitの入口・読込失敗復帰・実射撃15件成功。初期最大1,863,495B、練習まで最大7,440,902B。
- 元画像は1672×941のまま。解像度の再生成完了とは扱わない。ユーザー方針に従いカメラ倍率で粗さを緩和。
- 次工程: 最新のモバイル操作・75%倍率での画面確認、画像地形をオンラインを含む全マップへ展開するための素材/当たり判定整合。その後に原案と全シーンを照合する。
- 実機・実地域・実DO容量/費用、運用判断、正式art承認、本番公開は未完のまま。

2026-09-12 岩橋8人検証: 配信build/local SQLite DO、3browser各8contextで4v4入室→射撃共有→降参決着→全員帰還を確認。最大転送7,650,357B。Firefoxの開幕前操作可能判定を、時間表示だけでなくボタン状態を待つよう修正。details: evidence/ui/rock-arch-eight-2026-09-12。葦の丘v3の新素材・当たり判定候補も制作し、2〜8人配置/移動/洞窟内着地を検証したが、まだオンライン登録はしていない。
