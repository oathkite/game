...
| ブラウザーの戻る（21.7） | WorldScenesはReact stateとtimerで遷移。client/srcにpopstate・pushState・beforeunloadの処理なし。既存E2Eの戻るは画面内ボタンのみ | 未実装。メニューの戻り先、練習中の確認、オンライン退出の意味とroom.leave、招待の直入室を含む実ブラウザー検証が必要 |
