...
2026-09-26、ユーザー決定により正式名称を **TANK SHOOT** とする。
ロゴ・画面タイトル・紹介文ではTANK SHOOTに統一する。過去の参照画像や資料の固有タイトルは、素材を特定できるよう元の表記を保持する。
ロゴは画像を持たず、画面タイトルと同じドットフォントの文字で表す。
名称決定と商標・同名サービス・ドメインの利用可否確認は別であり、後者は未実施（TBD-36）。

旧名称 KEROPOD（2026-09-09 決定）は、2026-09-13 のドットテーマへの変更で取り下げた（36章）。
コードとデータの識別子（保存先の `.keropod/`、`CLIENT_BUILD` の `keropod-sim-v2.4` など）は、保存済みの状態や接続中のクライアントとの互換に関わるので変えない。

## 20.1 結論と設計の優先順位

TANK SHOOTを「URLを開けば、自分の小さなマシンで世界の人と一発を競えるゲーム」にする。
ブラウザを主要な提供先とし、TypeScript / React / PixiJSを継続する。ゲームエンジンの全面移行は現段階では推奨しない。
一方、2人固定モデル、全マップを収める表示、全ルームを集めたサーバー構成は置き換える。

優先する体験は、(1)一発の読み合い、(2)友人を誘いやすい入口、(3)多人数でも状況が分かること。
カメラや開始演出は、この三つを支える。世界公開を理由にショップ、ランキング、複雑な育成を先行させない。
...
ポトリス固有の画像・UI・音・名称は流用せず、既存のTANK SHOOTの方向性で制作する。

## 20.2 設計の正本と旧仕様の扱い

今回の新しい実装目標は20〜23章。従来の0〜10章は、変更対象外の仕様と現行実装の参照として残す。
衝突した場合、カメラ・多人数ルール・開始導線・通信構成は20〜23章を優先する。
アートは[36章](36-simple-dot-theme.md)のドットテーマに従う。
...
| 工程 | 成果物 | 次へ進む条件 |
|---|---|---|
| A: 体験の試作 | 実アセット1機、旧マップ、カメラ、上下HUDの操作可能な試作 | 844×390とPCで照準・パン・発射を実機確認 |
...
| E: 体験の完成 | タイトル、導線、全シーン遷移、音 | スキップ・低速回線・音声拒否・招待・復帰を確認 |
...
| 識別子 | 初期案 | 判断方法 |
|---|---|---|
| R-01 機体倍率 | 6 CSS px/cell、狭い横画面は4.5 | 実アセットの砲口、3画面サイズで目視 |
