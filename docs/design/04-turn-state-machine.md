...
- 武器の切り替え（スロット 0 か 1 か。クライアントで保持し、射撃確定時に送る）
- 射撃確定（向き、仰角、パワー、移動後の x、武器のスロットをまとめて送る）
...
設定メニューを開き「降参する」を押す 2 操作で確定し、別の確認画面は挟まない（3.5）。
...
idle、low-hp、destroy、wreck などはクライアントの演出状態であり、この章の対戦状態を増やさない。
表示 HP は現在再生中の着弾に従う。
大破の演出を待ってサーバーの手番や終了時刻を遅らせず、再接続では受信状態に対応する表示へ復帰する。
低 HP の煙と短い被弾や発射の反応は重ね合わせ、大破だけは一度再生して残骸へ移る（[38.3](38-motion-enrichment.md#383-機体) の C4、C5）。
