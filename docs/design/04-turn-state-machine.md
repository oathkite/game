...
stateDiagram-v2
    [*] --> Waiting
    Waiting --> Ready: 2 人目が入室
    Ready --> TurnStart: 両者が準備完了
    TurnStart --> Acting: 風を配信
    Acting --> Resolving: 射撃確定 / 制限時間切れ
    Resolving --> Replaying: 結果を配信
    Replaying --> TurnStart: 再生完了、決着なし
    Replaying --> Finished: 決着 / ターン上限 / 再接続待ちの期限切れ
    Acting --> Finished: 降参 / 再接続待ちの期限切れ
    Finished --> [*]
```

各状態の意味は次のとおりである。

- **Waiting**：部屋に 1 人だけいる。招待を待っている。
- **Ready**：2 人が揃った。マップとスポーンを配信し、両者の読み込み完了を待つ。
- **TurnStart**：手番側を決め、風を抽選する。歩数を全回復する。
...
- 角度の変更（クライアントで保持し、射撃確定時に送る）
- 射撃確定（角度、パワー、移動後の x をまとめて送る）
...
1. 送信者が手番側である。
2. 状態が Acting である。
3. 角度が 10 から 170 の整数である。
4. パワーが 0 から 100 の整数である。
5. 移動後の x が、移動前の x から歩数の上限（15 歩 × 1 セル = 15 セル）以内である。
