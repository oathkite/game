...
| プレビュー | 選んだ色で塗った戦車を、対戦と同じ倍率のマップの切れ端に置いて表示する（[グラフィック](./08-visual-direction.md) の 8.2）。戦車の形は武器では変わらないが、武器を選ぶとその武器を右へ 1 発撃つデモを流す。弾の大きさ、扇と時間差の発、尾、着弾の段は対戦と同じ規則（[武器](./10-weapons.md) の 10.5）、着弾の演出（静止、膨張、削り、明滅、消失、破片）と地形の削れ方は対戦の再生と同じ時間割（[画面と操作](./03-ui-and-input.md) の 3.9）で見せる。貫通の続きの段は前の段の穴を抜けた先で当たる。弾道は見た目だけの簡易なもので、標準砲が主砲の先端から右端までの 6 割の位置に落ちるように初速を決める |
