...
| パラメータ | 範囲 | 初期値 |
|---|---|---|
| 移動速度 | 0.25〜3倍、0.05刻み | 2.8倍 |
| 慣性の長さ | 0〜1000ms、20ms刻み。0でOFF | 1000ms |

移動速度はドラッグ・スワイプ・端スクロール・Shift＋矢印に反映する。
慣性はスワイプ解放と端スクロール終了時の減速時間。開始・終了が滑らかな共通曲線を使用する。
上記の480px/秒・約38pxは追加当初の値。現在の初期値は承認済みの2.8倍・1000ms。自動追従・手番への復帰時間は変更しない。
...
## 2026-09-10：開始・終了をさらに滑らかに調整

手番移動と手動の端スクロール加速を、5次のsmootherstepへ変更。
カーブの両端で1階・2階微分が0になり、開始と終了の変化を穏やかにする。
手動の慣性も同じカーブで速度を現在値から0へ下げ、設定時間で停止する。
慣性の移動量は速度曲線の積分から求め、フレームレートによる距離差をなくす。
従来の指数減衰より余韻が長くなる。速度2.8倍・慣性1000msでは、上限速度から
解放した場合の追加移動は672 CSS px。マップ境界や取消時は従来どおり停止する。
ドラッグ中の直接追従とキーボード入力は従来どおり。
