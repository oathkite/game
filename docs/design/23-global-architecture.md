...
2026-09-26 に、運営と公開の条件を次のとおり決めた。

- **運営主体**：株式会社インプレッシブ。利用規約とプライバシーポリシーに運営者として記載する。
- **対象年齢**：13歳以上。理由は表現ではなく、知らない相手と対戦し、表示名や匿名sessionのような識別子を扱うことにある。米国のCOPPAは13歳未満の子ども向けサービスに保護者の同意を求め、継続的な識別子も個人情報に含める。子ども向けかどうかは絵柄や宣伝からも判断されうるので、集めるデータを最小限に保つ。EUの国ごとの同意年齢（13〜16歳）を含め、地域を絞ったbetaの前に法務の確認を行う（TBD-34）。
- **マネタイズ**：公開初期は行わない。フリーゲームとして提供し、ショップ、通貨、広告を置かない。
- **問い合わせ**：画面内のフォームで受け付け、サーバーから運営のSlackへ送る。運営者のメールアドレスは画面、設計書、リポジトリに載せない。このリポジトリは公開されているためである。返信に使うアドレスはTBD-35とする。
- **ロゴ**：画像を持たず、TANK SHOOTの文字で表す（20章）。
...
購入は初期公開から切り離し、導入時に所有・返金・復元を設計する。
