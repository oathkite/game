...
ここでは共有する対戦状態を定める。
本番の Durable Objects による運用状態の保存と、将来の外見所有権の保存は区別する。
...
type SeatStats = {
  ...
  readonly directHits: number;    // 着弾距離が 0 だった回数（6.7 のダメージの定義と同じ）
};
```
...
| 固定小数点の小数部 | 16 bit |
| 積分の刻み | 1/60 秒相当 |
| 重力 | 固定小数点で 2621。約 0.04 セル/step^2 |
| 風の作用 | 風 1 につき固定小数点で 20。約 0.0003 セル/step^2 |
| パワーから初速への換算 | パワー 100 で固定小数点 229376。3.5 セル/step。パワー p では 229376 × p / 100 を切り捨て |
...
初速を 3.5 セル/step に上げると到達距離は 313 セルになり、滞空が延びる分だけ風のずれが 33 セルまで広がるので、風の作用を 26 から 20 に下げて風 10 のずれを 24 セルに戻した。
数値の当たり方は client の単体動作で遊んで見直す。

**地形の判定は整数セルで行う。**
弾の位置を切り捨てで整数セルに丸めてマスクを見る。
1 ステップで複数セルを進む場合は、前のセルから新しいセルまでを、縦か横に 1 セルずつ進む 4 連結の整数直線で辿り、通る順に各セルを調べる。
対角に進む 8 連結の直線だと、1 セル幅の斜めの地形をすり抜けるためである。
...
この対称性はテストで固定する。
ただし弾の位置がセルの境界にちょうど乗った場合は、セルの区切りが半開区間であるために左右で 1 セルずれることがある。
これは避けられない性質として受け入れ、テストは境界に乗らない入力で行う。
...
撃つ側の x は `TrajectoryInput` の x（移動後）を正とし、渡された状態の x が移動前でも入力の値で上書きする。
奈落に落ちている機体は当たり判定を持たず、弾が外れた場合でもリングアウトとして判定する。
