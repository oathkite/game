...
type PlayerState = {
  ...
  readonly hp: number;
  readonly x: number;                 // 機体中心の x（整数 px）
...
| 固定小数点の小数部 | 8 bit |
| 積分の刻み | 1/60 秒相当 |
| 重力 | TBD-8（初期案は 1 ステップあたり下向き 0.15 px/step^2 相当） |
| 風の作用 | TBD-8（初期案は風 1 につき 1 ステップあたり 0.0015 px/step^2 相当） |
| パワーから初速への換算 | TBD-8（初期案はパワー 100 で 12 px/step） |

重力、風、初速の 3 つは互いに依存し、マップの大きさと合わせて「最大パワーで画面の 8 割を飛ぶ」「風 10 で着弾が 100 px 程度ずれる」という感触を目標に調整する。
初期案の根拠は次の概算である。
初速 12 px/step を 45 度で撃つと鉛直成分は約 8.5 px/step、重力 0.15 なら滞空は約 113 ステップになる。
風 10 で 0.015 px/step^2 の横加速が 113 ステップ続くと、ずれは約 96 px になる。

**地形の判定は整数ピクセルで行う。**
弾の位置を整数に丸めてマスクを見る。
1 ステップで複数ピクセルを進む場合は、前の位置との間を 1 px ずつ辿り、最初に地面か機体に触れた点を着弾点とする。
この辿り方も仕様として固定する。

**地形の削りは整数の円で行う。**
爆心と半径から、マスク上のどのピクセルを消すかは整数演算だけで決める（`dx*dx + dy*dy <= r*r`）。
